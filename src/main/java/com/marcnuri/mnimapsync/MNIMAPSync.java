...
public class MNIMAPSync {
	...
	public final void sync() {
		...
		try {
			...
			sourceCopier = new StoreCopier( sourceStore, sourceIndex, targetStore, targetIndex, syncOptions.getThreads() );
			sourceCopier.copy();
			...
			if ( syncOptions.getDelete() && sourceIndex != null && !sourceCopier.hasCopyException() ) {
				...
				sourceStore = openStore( syncOptions.getHost1(), syncOptions.getThreads() );
				targetStore = openStore( syncOptions.getHost2(), syncOptions.getThreads() );
				targetDeleter = new StoreDeleter( sourceStore, sourceIndex, targetStore, syncOptions.getThreads() );
	...
	// Static Methods
	// **************************************************************************************************
	public static final String translateFolderName(
	...
	public static void main(
		...
		try {
			...
			if ( sync.sourceCopier != null ) {
				System.out.printf( "Folders copied: %s/%s", sync.sourceCopier.getFoldersCopiedCount(), ( sync.sourceCopier.getFoldersCopiedCount() + sync.sourceCopier.getFoldersSkippedCount() ) );
				System.out.printf( "\nMessages copied: %s/%s", sync.sourceCopier.getMessagesCopiedCount(), ( sync.sourceCopier.getMessagesCopiedCount() + sync.sourceCopier.getMessagesSkippedCount() ) );
				System.out.printf( "\nSpeed: %.2f m/s", ( ( sync.sourceCopier.getMessagesCopiedCount() + sync.sourceCopier.getMessagesSkippedCount() ) / ( (double) sync.getElapsedTimeInSeconds() ) ) );
				System.out.printf( "\nExceptions: %s", sync.sourceCopier.hasCopyException() );
			}
			if ( sync.targetDeleter != null ) {
				System.out.printf( "\nFolders deleted: %s/%s", sync.targetDeleter.getFoldersDeletedCount(), ( sync.targetDeleter.getFoldersDeletedCount() + sync.targetDeleter.getFoldersSkippedCount() ) );
				System.out.printf( "\nMessages deleted: %s/%s", sync.targetDeleter.getMessagesDeletedCount(), ( sync.targetDeleter.getMessagesDeletedCount() + sync.targetDeleter.getMessagesSkippedCount() ) );
			}
	 ...
	 * @return
	 */
	private static SyncOptions parseArgs(
			String[] args,
			SyncOptions options,
			int current ) {
	 ...
	 */
	private static IMAPStore openStore(
			HostDefinition host,
		...
		if ( host.isSsl() ) {
			...
			properties.put( "mail.imaps.socketFactory.class", AllowAllSSLSocketFactory.class.getName() );
	...
	private static final class SyncMonitor extends TimerTask {
		...
		public void run() {
			...
			System.out.print( String.format( "\rIndexed (target): %,d/%,d  Copied: %,d/%,d " + "Deleted: %,d/%,d Speed: %.2f m/s", ( sync.targetIndex != null ? sync.targetIndex.getIndexedMessageCount() : 0l ),
					( sync.targetIndex != null ? sync.targetIndex.getIndexedMessageCount() + sync.targetIndex.getSkippedMessageCount() : 0l ),
					( sync.sourceCopier != null ? sync.sourceCopier.getMessagesCopiedCount() : 0 ), ( sync.sourceCopier != null ? sync.sourceCopier.getMessagesCopiedCount() + sync.sourceCopier.getMessagesSkippedCount()
							: 0 ), ( sync.targetDeleter != null ? sync.targetDeleter.getMessagesDeletedCount() : 0 ),
					( sync.targetDeleter != null ? sync.targetDeleter.getMessagesDeletedCount() + sync.targetDeleter.getMessagesSkippedCount() : 0 ),
					( sync.sourceCopier != null ? (double) ( sync.sourceCopier.getMessagesCopiedCount() + sync.sourceCopier.getMessagesSkippedCount() ) / (double) sync.getElapsedTimeInSeconds() : 0 ) ) );
		}
